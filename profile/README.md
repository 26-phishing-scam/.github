...
flowchart TD
    ...
    A["Chrome Extension<br/><br/>URL 감지<br/>입력 이벤트 감지<br/>다운로드 이벤트 감지<br/>결제 / 로그인 감지"]

    B["FastAPI Server<br/><br/>이벤트 수신<br/>이벤트 검증<br/>위험 사유 분석<br/>요약 데이터 제공"]

    C["In-Memory Storage<br/><br/>최근 이벤트 저장<br/>최근 도메인 저장<br/>요약 정보 생성"]
